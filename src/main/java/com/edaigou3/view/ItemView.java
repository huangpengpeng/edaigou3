...
public class ItemView extends BaseViewAdapter {
							...
							public void completed(IBrowserView browserView) {
								try {
									...
									if (!(Boolean) result.get("ok")) {
										MessageBox2.showErrorMsg(
												"获取失败,检查参数是否正确", null);
										return;
									}
									Item item = toModel(result);
									if (item == null) {
										MessageBox2.showErrorMsg("此商品没有淘客",
												null);
										return;
									}
									NewInstance.get(ItemView.class)
											.fullContents(item);
								} catch (Exception e) {
	...
	protected Item toModel(Map<String, Object> result)
		...
		item = new Item(null, imageByteValue, null, title, auctionUrl,
				Long.valueOf(tbkNumIid), null, rebateProportion, rebateFee,
				rebateFee.multiply(new BigDecimal("0.1")), null, null, null,
				null, null, null, null);
		return item;
	}
}
